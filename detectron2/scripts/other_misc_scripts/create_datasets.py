...
def include_depth_in_dataset(imagesPath="/home/clara/datasets/rgb/train", depthsPath = "/home/clara/datasets/depth/train", 
                             outPath = "/home/clara/datasets", datasetFunction = "train", extension = "png",
                             saveDepth = True, showAll = True, saveRGBD = True,
    ...
    for c in range(len(colorFormatNames)):
        ...
        for c2 in canals[c]:
            if not os.path.isdir(outPath+"/"+cfn+str(c2)): os.mkdir(outPath+"/"+cfn+str(c2))
            if not os.path.isdir(outPath+"/"+cfn+str(c2)+"/"+datasetFunction): os.mkdir(outPath+"/"+cfn+str(c2)+"/"+datasetFunction)

    if saveDepth:
        if not os.path.isdir(outPath+"/"+"depth_normalized"): os.mkdir(outPath+"/"+"depth_normalized")
        if not os.path.isdir(outPath+"/"+"depth_normalized"+"/"+datasetFunction): os.mkdir(outPath+"/"+"depth_normalized"+"/"+datasetFunction)

    if saveRGBD:
        if not os.path.isdir(outPath+"/"+"RGBD"): os.mkdir(outPath+"/"+"RGBD")
        if not os.path.isdir(outPath+"/"+"RGBD"+"/"+datasetFunction): os.mkdir(outPath+"/"+"RGBD"+"/"+datasetFunction)

    if showAll:
        if not os.path.isdir(outPath+"/"+"all"): os.mkdir(outPath+"/"+"all")
        if not os.path.isdir(outPath+"/"+"all"+"/"+datasetFunction): os.mkdir(outPath+"/"+"all"+"/"+datasetFunction)
    ...
    for i in range(len(imagesPaths)):
        ...
        if showAll: 
            totalImage = np.hstack((image, np.dstack((normalized_depth, normalized_depth, normalized_depth)), colorized_depth))
        if saveDepth:
            Image.fromarray(normalized_depth).save(outPath+"/"+"depth_normalized"+"/"+datasetFunction+"/"+name)
        ...
        if saveRGBD:
            ...
            Image.fromarray(RGBD).save(outPath+"/"+"RGBD"+"/"+datasetFunction+"/"+name+".png")
        for c in range(len(colorFroms)):
            ...
            for c2 in canals[c]:
                ...
                Image.fromarray(depthImage).save(outPath+"/"+colorFormatNames[c]+str(c2)+"/"+datasetFunction+"/"+name)
                if showAll:
                    lineImage = np.hstack((lineImage, depthImage))
            if showAll:
                totalImage = np.vstack((totalImage, lineImage))
        if showAll:
            Image.fromarray(totalImage.astype(np.uint8)).save(outPath+"/"+"all"+"/"+datasetFunction+"/"+name)
...
def normalize_depth(depth, reverse = True, percentileForMin = 5, percentileForMax = 85, 
