...
class DoubleMPViT(Backbone):
    ...
    def __init__(self, mpvit1: MPViT_Backbone, mpvit2: MPViT_Backbone, out_features=None, fusion_steps = [], attention=False, relu=False, bias=False, fuse_in = "DEPTH"):
        super().__init__()
        self.mpvit1 = mpvit1
        self.mpvit2 = mpvit2
        ...
        if "stem" in fusion_steps or "stem" in out_features:
            fusion_features_dict["stem"] = [mpvit1.backbone.stem.out_channels, mpvit2.backbone.stem.out_channels]
        for i, name in enumerate(self.stage_names):
            fusion_features_dict[name] = [mpvit1.backbone.mhca_stages[i].out_channels, mpvit2.backbone.mhca_stages[i].out_channels]
    ...
    def forward(self, x1, x2):
        ...
        """
        assert x1.dim() == 4 and x2.dim() == 4, f"MPViT takes an input of shape (N, C, H, W). Got {x1.shape} and {x2.shape} instead!"
        outputs = {}
        ...
        if self.fuse_in=="DEPTH":
            ...
            for name, patch_stage1, patch_stage2, stage1, stage2 in zip(self.stage_names, 
...
def build_double_mpvit_backbone(cfg, input_shape1, input_shape2):
    ...
    return DoubleMPViT(mpvit1, mpvit2, out_features=out_features, 
