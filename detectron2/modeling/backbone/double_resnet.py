...
class DoubleResNet(Backbone):
    ...
    def __init__(
        ...
        if "stem" in fusion_steps or "stem" in out_features:
            fusion_features_dict["stem"] = [
                resnet_color.stem.out_channels,
                resnet_depth.stem.out_channels,
            ]
        for i, name in enumerate(self.stage_names):
            fusion_features_dict[name] = [
                resnet_color.stages[i].out_channels,
                resnet_depth.stages[i].out_channels,
            ]
        ...
        for name, out_channels in fusion_features_dict.items():
            if name in out_features:
                self.fusion_layers[name] = FusionLayer(
                    out_channels[0],
                    out_channels[1],
                    out_channels[1] if fuse_in == "DEPTH" else out_channels[0],
                    attention=attention,
                    relu=relu,
                    bias=bias,
                )
    ...
    def forward(self, x_color, x_depth):
        ...
        """
        assert (
            x_color.dim() == 4 and x_depth.dim() == 4
        ), f"ResNet takes an input of shape (N, C, H, W). Got {x_color.shape} and {x_depth.shape} instead!"
        outputs = {}
        ...
        if self.fuse_in == "DEPTH":
            ...
            for name, stage1, stage2 in zip(
                self.stage_names, self.resnet_color.stages, self.resnet_depth.stages
            ):
...
def build_double_resnet_backbone(cfg, input_shape1, input_shape2):
    ...
    cfg.MODEL.BACKBONE.FREEZE_AT = cfg.MODEL.BACKBONE.get(
        "FREEZE_AT_DEPTH", cfg.MODEL.BACKBONE.FREEZE_AT
    )
    cfg.MODEL.RESNETS.NUM_GROUPS = cfg.MODEL.RESNETS.get(
        "NUM_GROUPS_DEPTH", cfg.MODEL.RESNETS.NUM_GROUPS
    )
    cfg.MODEL.RESNETS.WIDTH_PER_GROUP = cfg.MODEL.RESNETS.get(
        "WIDTH_PER_GROUP_DEPTH", cfg.MODEL.RESNETS.WIDTH_PER_GROUP
    )
    cfg.MODEL.RESNETS.STEM_OUT_CHANNELS = cfg.MODEL.RESNETS.get(
        "STEM_OUT_CHANNELS_DEPTH", cfg.MODEL.RESNETS.STEM_OUT_CHANNELS
    )
    cfg.MODEL.RESNETS.RES2_OUT_CHANNELS = cfg.MODEL.RESNETS.get(
        "RES2_OUT_CHANNELS_DEPTH", cfg.MODEL.RESNETS.RES2_OUT_CHANNELS
    )
    cfg.MODEL.RESNETS.STRIDE_IN_1X1 = cfg.MODEL.RESNETS.get(
        "STRIDE_IN_1X1_DEPTH", cfg.MODEL.RESNETS.STRIDE_IN_1X1
    )
    cfg.MODEL.RESNETS.RES5_DILATION = cfg.MODEL.RESNETS.get(
        "RES5_DILATION_DEPTH", cfg.MODEL.RESNETS.RES5_DILATION
    )
    cfg.MODEL.RESNETS.DEFORM_ON_PER_STAGE = cfg.MODEL.RESNETS.get(
        "DEFORM_ON_PER_STAGE_DEPTH", cfg.MODEL.RESNETS.DEFORM_ON_PER_STAGE
    )
    cfg.MODEL.RESNETS.DEFORM_MODULATED = cfg.MODEL.RESNETS.get(
        "DEFORM_MODULATED_DEPTH", cfg.MODEL.RESNETS.DEFORM_MODULATED
    )
    cfg.MODEL.RESNETS.DEFORM_NUM_GROUPS = cfg.MODEL.RESNETS.get(
        "DEFORM_NUM_GROUPS_DEPTH", cfg.MODEL.RESNETS.DEFORM_NUM_GROUPS
    )
