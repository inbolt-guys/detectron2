...
def normalize_depth(
    ...
    if not min_depth:
        if method == "percentile":
            min_depth = (
                np.percentile(depth[np.logical_not(noValueMask)], percentileForMin)
                * scaleFactorForMin
            )
        elif method == "real":
...
wget.download(
    "https://huggingface.co/spaces/MykolaL/evp/resolve/main/depth/nyu_class_embeddings_my_captions.pth"
)
