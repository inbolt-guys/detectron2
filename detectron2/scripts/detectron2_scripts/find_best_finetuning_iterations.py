...
for model_name in model_names:
    ...
    plt.savefig(
        os.path.join(
            model_path, f"metrics_thresh_{cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST}_finetuned.png"
        )
    )

    with open(
        os.path.join(
            model_path, f"eval_thresh_{cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST}_finetuned.json"
        ),
        "w",
    ) as out_file:
