...
def build_mpvit_backbone(cfg, input_channels: int = 3):
    """
    ...
    model_kwargs.update({"in_chans" : input_channels})
...
@BACKBONE_REGISTRY.register()
def build_mpvit_double_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Create a MPViT w/ 2 FPN backbone for color and depth.
    Args:
        cfg: a detectron2 CfgNode
    ...
    if cfg.INPUT.FORMAT == "RGBD" or cfg.INPUT.FORMAT == "RGBRD":
        bottom_upRGB = build_mpvit_backbone(cfg, input_channels=3)
        freeze_at = cfg.MODEL.BACKBONE.FREEZE_AT
        cfg.MODEL.BACKBONE.FREEZE_AT = cfg.MODEL.BACKBONE.FREEZE_AT_DEPTH
        bottom_upDepth = build_mpvit_backbone(cfg, input_channels=1)
        cfg.MODEL.BACKBONE.FREEZE_AT = freeze_at
    elif cfg.INPUT.FORMAT == "GN":
        bottom_upRGB = build_mpvit_backbone(cfg, input_channels=1)
        cfg.MODEL.BACKBONE.FREEZE_AT = cfg.MODEL.BACKBONE.FREEZE_AT_DEPTH
        bottom_upDepth = build_mpvit_backbone(cfg, input_channels=3)
        cfg.MODEL.BACKBONE.FREEZE_AT = freeze_at
    else:
