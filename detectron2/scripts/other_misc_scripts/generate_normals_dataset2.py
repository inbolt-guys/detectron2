...
def process_images_in_directory(dataset, num_processes):
    images = sorted(glob.glob(f"/home/clara/detectronDocker/dataset_for_detectron/coco2017_depth/{dataset}/*.png"))
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [executor.submit(process_images_in_directory, dataset, num_processes) for dataset in datasets]
