...
if __name__ == "__main__":
    ...
    cfg_detectron.SOLVER.STEPS = (int(270_000*(16/bs)), int(310_000*(16/bs)))
    cfg_detectron.SOLVER.MAX_ITER = int(340_000*(16/bs))
    cfg_detectron.SOLVER.IMS_PER_BATCH = bs

    cfg_detectron.MODEL.FREEZE_INCLUDE = ["res"]
    cfg_detectron.MODEL.UNFREEZE_SCHEDULE = [["backbone.bottom_up.res"]]
    cfg_detectron.MODEL.UNFREEZE_ITERS = [80_000]
    ...
    # paths
    pipeline = Pipeline(
        output_dir="/app/detectronDocker/outputs/RCNN_RGB_class_agnostic_and_normalized_1class_freeze_all_except_stem",
        cfg=cfg_detectron,
        hardware=hardware,
    )
    pipeline.register_dataset("coco_2017_depth_train", 
                              coco_train_folder, annotations_file="/app/detectronDocker/dataset_for_detectron/coco2017_depth/class_agnostic_annotations/instances_train2017.json")
    pipeline.register_dataset("coco_2017_depth_val", 
                              coco_val_folder, annotations_file="/app/detectronDocker/dataset_for_detectron/coco2017_depth/class_agnostic_annotations/instances_val2017.json")
