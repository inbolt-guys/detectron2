...
parser = argparse.ArgumentParser(
    description="""This script is designed to quantize the Backbone OpenVINO model (IR Representation = .xml file + .bin file)
                                 using a calibration dataset. It is exactly the same script as the quantize.py script. The calibration dataset should be around of 300 images that can be taken from training and validatation 
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="File path of the .xml file (in the same folder as the .bin file)",
)
