...
for d in datasets:
    image_paths = sorted(glob.glob(f"/home/inbolt/data_hdd/R&D/clara/datasets/SceneNet/{d}/*/*/photo/*.jpg"))
    ...
    for ip in image_paths:
        ...
        depth = cv2.imread(os.path.join(parent_folder, "depth", im_name).replace("jpg", "png"), cv2.IMREAD_UNCHANGED)
        segm = cv2.imread(os.path.join(parent_folder, "instance", im_name).replace("jpg", "png"), cv2.IMREAD_UNCHANGED)

        rgbd = np.dstack((rgb.astype(np.uint16), depth.astype(np.uint16)))
        rgbd_name = os.path.basename(os.path.dirname(parent_parent_folder))+"_" + \
                os.path.basename(parent_folder)+"_"+im_name.replace("jpg", "png")
        ...
        for object in per_object_masks:
            annotation_id += 1
            annotations.append(coco_utils.create_annotation_format(image_id, 1, annotation_id, 
