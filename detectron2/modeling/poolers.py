...
__all__ = ["ROIPooler", "ROIDoubleBackbonePooler"]
...
def assign_boxes_to_levels(
    ...
    """if box_second_backbone.nonzero().numel() > 0:
        level_assignments = torch.floor(
            canonical_level + box_second_backbone * nbLevels + torch.clamp(torch.log2(box_sizes / canonical_box_size + 1e-8), -canonical_level, max_level-canonical_level)
        )
...
class ROIDoubleBackbonePooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        for level, pooler in enumerate(self.level_poolers):
            ...
            pooler_fmt_boxes_level = pooler_fmt_boxes[inds]
            # Use index_put_ instead of advance indexing, to avoid pytorch/issues/49852
            pooler_result = pooler(x[level], pooler_fmt_boxes_level)
            output.index_put_((inds,), pooler_result)
