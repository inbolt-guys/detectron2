...
register_dataset("OCID", ocid_train_folder, annotations_file=ocid_train_annos)
all_results = {}
...
for model_name in models:
    ...
    res = trainer.test(
        cfg=cfg, model=trainer.model, evaluators=evaluator, return_inference_time=True
    )
    print(res)
    all_results[model_name] = res

with open(
    os.path.join("/app/detectronDocker/outputs", f"all_results_OCID_40.json"), "w"
) as out_file:
