...
parser = argparse.ArgumentParser(description="""This script can be used to convert a detectron2 trained model to OpenVINO model for deployement. To use this script 
...
parser.add_argument('--folder_path',type= str,  required=True,help="Folder containing the .pth file and the .yaml file")
parser.add_argument('--input_width',type= int, required=True, help="Input width of the OpenVINO model. Make sure this input size has been trained")
parser.add_argument('--input_height',type= int, required=True, help="Input height of the OpenVINO model. Make sure this input size has been trained")
parser.add_argument('--image_path',type= str, required=True, help="Path of an image to do the tracing conversion to OpenVINO. You can take one picture of the training or validation dataset")
...
cfg.INPUT.IMAGE_SIZE_AFTER_RESIZE = [(args.input_width,args.input_height)] # [(640,853)] # New parameter of the config file added in Maxime's commit 
...
output_file = script_dir + '/' +  'detectron2_rgbrd_' + str(args.input_width) +'_' +  str(args.input_height) + '.xml'
