...
for d in ["data"]:
    ...
    if d == "data":
        image_paths = sorted(glob.glob(f"/home/inbolt/data_hdd/R&D/clara/datasets/YCB/{d}/00*/*-color.jpg"))
    else:
        image_paths = sorted(glob.glob(f"/home/inbolt/data_hdd/R&D/clara/datasets/YCB/{d}/*-color.jpg"))
    ...
    for ip in image_paths:
        ...
        h, w = rgb.shape[:2]
        if is_train:
            images_annotations["train"].append(coco_utils.create_image_annotation(rgbd_name, w, h, image_id["train"]))
        else:
            images_annotations["test"].append(coco_utils.create_image_annotation(rgbd_name, w, h, image_id["test"]))
